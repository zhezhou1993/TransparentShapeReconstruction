...
parser.add_argument('--re', type=int, default=16, help='the end id of the shape' )
...
for shape in shapeList:
    cam_file = [f for f in os.listdir(shape) if f.endswith('txt')][0]
    nVHviews = int(cam_file.replace('cam', '').replace('.txt', '')) 
    nameVH = 'visualHullSubd_{}.obj'.format(nVHviews )
    cnt += 1
    print('Processing %d/%d' % (cnt, len(shapeList) ), shape )
    pointsVHName = osp.join( shape, nameVH.replace('.obj', '_pts.npy') )
    normalsVHName = osp.join( shape, nameVH.replace('.obj', '_ptsNormals.npy') )
    pointsGtName = osp.join( shape, nameGt.replace('.obj', '_%d_pts.npy' % nVHviews ) )
    normalsGtName = osp.join( shape, nameGt.replace('.obj', '_%d_ptsNormals.npy' % nVHviews ) )
